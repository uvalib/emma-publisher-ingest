...
def main():
	...
	if args.opensearch_url:
		print('sending records directly to opensearch at url '+args.opensearch_url)
		opensearch_url = args.opensearch_url
	else :
		opensearch_url = os.getenv(OPENSEARCH_URL, DEFAULT_OPENSEARCH_HOST)

	if args.index_name:
		print('sending records directly to opensearch index '+args.index_name)
		index = args.index_name
	else : 
		index = os.getenv(OPENSEARCH_INDEX, DEFAULT_OPENSEARCH_INDEX)
		
	if args.opensearch_url:
		globals.opensearch_conn = OpenSearchConnection(opensearch_url, index)
